package com.example.andoridmediaplayerilendirilenmziidinlemeprogressdialogdevam;
...
indirdikce dolacak bir çizgi gibi düşün bunu yapcak şey PROGRESSDİALOG
   ...
   UZUN İŞLEMLER İÇİN SERVİSLER KULLANILIR*/
    ...
    public  void müzikDinle(){
        //Müzikleri dinleyebilmek için MediaSınıfı olusturucaz

        Uri myUri=Uri.parse(Environment.getExternalStorageDirectory().getPath()+"/sila.mp3");
        //Yoluumuzu belirledik

        mPlayer=new MediaPlayer();
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        //mÜZik için type bu şekilde

        try {
            mPlayer.setDataSource(context,myUri);

            mPlayer.prepare();
            //Müzik çalmadan önce bu gerekli

            mPlayer.start();//Müzik Çalıyor

            //Müzik bittikten sonra ne olsun
            mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                @Override
                public void onCompletion(MediaPlayer mp) {
                    Toast.makeText(context,"Müzik Dinlenmesi Bitti",Toast.LENGTH_LONG).show();
